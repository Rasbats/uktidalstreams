 ...
 * CrossHairOverlay scaleBar = new CrossHairOverlay(map); // Thiw is an important change of calling!
 *
...
public class CrossHairOverlay extends Overlay implements GeoConstants {
	...
	public CrossHairOverlay(final MapView mapView) {
		super();
	...
	public void draw(Canvas c, MapView mapView, boolean shadow) {
		...
		if (zoomLevel >= minZoom) {
			...
			if (centred && latitudeBar)
				offsetX += 0; //-latitudeBarRect.width() / 2;
			if (centred && longitudeBar)
				offsetY += 0;//-longitudeBarRect.height() / 2;
...
/*
			...
			if (longitudeBar && bgPaint != null) {
				...
				int offsetTop = 0; //latitudeBar ? latitudeBarRect.height() : 0;
	...
	protected void rebuildBarPath(final Projection projection) {   //** modified to protected
		...
		final int xBarLengthPixels = (int) (xLen * xMetersAdjusted / xMeters/4);
		// Divide by 4 to reduce length (final)
		...
		// get adjusted length in pixels
		final int yBarLengthPixels = (int) (yLen * yMetersAdjusted / yMeters/4);
		...
		if (latitudeBar) {
			...
			barPath.lineTo(barToX-xBarLengthPixels*2, barOriginY);
			barPath.lineTo(barOriginX, barOriginY);

			//latitudeBarRect.set(barOriginX, barOriginY, barToX, barOriginY);
		}

		if (longitudeBar) {
			// draw longitude bar

			barPath.lineTo(barOriginX, barToY-yBarLengthPixels*2);
			barPath.lineTo(barOriginX, barToY);

			//longitudeBarRect.set(barOriginX, barOriginY, barOriginX, barToY);
		}
	}
