...
final public class TideComp {
    ...
    //boolean indexFileValid = false;                 i
    //boolean indexValid = false;
    TreeMap<String, TreeMap> root = new TreeMap();
    String tideDataFile;
    SiteSet siteSet;

    public List<Double> myHWLW = new ArrayList<Double>();
    public List<Long> myHighWaterTimes = new ArrayList<Long>();

    TideComp(MapActivity m) {
        siteIndex = new ArrayList();
        app = m;//(TidePredictorApplication) main.getApplication();
        tideDataFile = "harmonics/harmonics.txt";
        siteSet = new SiteSet();
    }
    ...
    long setDT(long t) {
        ...
        siteSet.daylightInEffect = false; //((app.configValues.daylightTime == 2) | ((app.configValues.daylightTime == 1) & dst));
    ...
    String formatDate(GregorianCalendar pos, boolean database, boolean timeOnly, boolean includeYear, boolean roundUp, String delim) {
        ...
        if (database) { // need seconds
            ...
            if (timeOnly) {
                ...
                result = TideConstants.dowNames[pos.get(Calendar.DAY_OF_WEEK) - Calendar.SUNDAY] + " " + padChar(pos.get(Calendar.DAY_OF_MONTH), 2, "0") + "/" + padChar((pos.get(Calendar.MONTH) + 1), 2, "0") + yearStr + " " + padChar(pos.get(Calendar.HOUR_OF_DAY), 2, "0") + ":" + padChar(pos.get(Calendar.MINUTE), 2, "0") ;
        ...
        for (long t = start; t <= end && (threadStopper == null || !threadStopper.stop); t += 960) {
            ...
            if ((sign = height > 0) != oldSign) {
                ...
                if (te.t >= bt && te.t <= et) {
                    results.add(te);
                    if (te.high) {
                        myHighWaterTimes.add(te.t);
                    }
                    myHWLW.add(te.height);// for calculating ranges
                }
            }
            oldSign = sign;
            oldTime = t;
    ...
    void buildNewIndex() {
        ...
        if (app.harmonicArray == null) {
            ...
            app.decodedHarmonics = harmonicsToArrayList(app.harmonicArray);
            ...
            int i = 0;
            for (String s : app.decodedHarmonics) {
    ...
    //My calendar link bits
    public ArrayList getHWFromCalendar(long myCalendarTime, String portName){

        buildNewIndex();
        int sz = app.reverseArray.size();
        boolean found = false;
        ArrayList<String> myHW = new ArrayList<String>();
        for (int i = 0; i< sz; i++){
            String s = app.titleArray.get(i);
            if (s.contains(portName) && (s.contains("England")||s.contains("Scotland")||s.contains("Channel Islands")||s.contains("Wales")) ) {
                app.configValues.lastDisplayedSite = i;
                found = true;
                break;
            }
        }
        ...
        long tm = myCalendarTime;
        siteSet.epochTime = tm / 1000;
        siteSet.currentYear = 2017;

        processSiteData(app.configValues.lastDisplayedSite);
        long t = myCalendarTime/1000 - (7*3600);
        long endt = t + (24) * 3600;
        long prevt = getNextEventTime(siteSet, t, false); // previous
        long postt = getNextEventTime(siteSet, endt, true); // next
        ArrayList eventList = predictTideEvents(siteSet, prevt, postt, t, endt, null);
        ArrayList data = eventList;


        for (int i = 0; i<data.size(); i++) {
            String text = formatDataString(i, data, false, false, "", true, false, true, "");
            myHW.add(i, text);
       }
